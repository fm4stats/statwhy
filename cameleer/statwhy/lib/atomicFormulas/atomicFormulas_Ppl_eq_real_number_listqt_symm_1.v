...
(* Why3 goal *)
Theorem eq_real_number_list'_symm :
...
- induction p2.
  + intros.
    exact H.
  + intros.
    set (F := eq_real_number_list''def w nil (a :: p2)).
    simpl in F.
    contradict (F H).
- induction p2.
  + intros.
    exact H.
  + intros.
    set (F := eq_real_number_list''def w (a :: p2) nil).
    simpl in F.
    contradict (F H).
- split.
  + destruct p2.
    * intro.
      set (F := eq_real_number_list''def w (a :: p1) nil).
      simpl in F.
      contradict (F H).
    * intros.
      set (F := eq_real_number_list''def w (a :: p1) (r :: p2)).
      simpl in F.
      apply F in H.
      destruct H as [H1 H2].
      rewrite Real_equal_symm in H1.
      set (G := eq_real_number_list''def w (r :: p2) (a :: p1)).
      simpl in G.
      rewrite -> IHp1 in H2.
      assert (E : infix_breq w (infix_dleq (Real r) (Real a)) /\ eq_real_number_list' w p2 p1).
      {
        split; auto.
      }
      rewrite <- G in E.
  ...
  + destruct p2.
    * intro.
      set (F := eq_real_number_list''def w nil (a :: p1)).
      simpl in F.
      contradict (F H).
    * intros.
      set (F := eq_real_number_list''def w (r :: p2) (a :: p1)).
      simpl in F.
      apply F in H.
      destruct H as [H1 H2].
      rewrite Real_equal_symm in H1.
      set (G := eq_real_number_list''def w (a :: p1) (r :: p2)).
      simpl in G.
      rewrite <- IHp1 in H2.
      assert (E : infix_breq w (infix_dleq (Real a) (Real r)) /\ eq_real_number_list' w p1 p2).
      {
        split; auto.
      }
      rewrite <- G in E.
