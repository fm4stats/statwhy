...
(* Why3 assumption *)
Fixpoint is_const_list
  (ss:Init.Datatypes.list real_number) {struct ss}: Init.Datatypes.bool :=
  match ss with
  | Init.Datatypes.nil => Init.Datatypes.true
  | Init.Datatypes.cons s ss' =>
      match s with
      | Const _ => is_const_list ss'
      | _ => Init.Datatypes.false
      end
  end.

(* Why3 assumption *)
Definition is_known (ppl:population) : Prop :=
  match ppl with
  | NormalD s1 s2 =>
      match (s1, s2) with
  ...
  | CategoricalD ss => ((is_const_list ss) = Init.Datatypes.true)
  | _ => False
  end.
...
Axiom know_implies_statb :
  forall (w:world) (p:pvalue) (h:formula), infix_breq w (Know h) ->
  infix_breq w (StatB p h).
...
Axiom bernoullip_def :
  ...
  infix_breq w
  (infix_dleq
   (bernoullip (CategoricalD (Init.Datatypes.cons p Init.Datatypes.nil)))
   (Real p)).
...
Parameter s: Init.Datatypes.list Reals.Rdefinitions.R.

Parameter i: real_term -> Reals.Rdefinitions.R.

(* Why3 goal *)
Theorem is_prob_dist_const_implies_is_prob_dist_const :
  ((fold
    (fun (y0:Reals.Rdefinitions.R) (y1:Reals.Rdefinitions.R) => (y0 + y1)%R)
    0%R s)
   =
   (fold
    (fun (acc:Reals.Rdefinitions.R) (r:real_number) =>
     (acc + (i (Real r)))%R)
    0%R (map (fun (r:Reals.Rdefinitions.R) => Const r) s))).
Proof.
intros.
