...
Axiom Ppl_check_variance_normald_param :
  forall (mean1:real_number) (s:Strings.String.string) (w:world),
  ~ infix_breq w (check_variance (NormalD mean1 (Param s))).

Parameter rliteral9: Strings.String.string.

Axiom rliteral_axiom9 : True.

(* Why3 assumption *)
Definition eq_variance (p1:population) (p2:population) : formula :=
  Atom
  (Pred rliteral9
   (Init.Datatypes.cons (PopulationT p1)
    (Init.Datatypes.cons (PopulationT p2) Init.Datatypes.nil))).

Axiom eq_variance_def :
  forall (p1:population) (p2:population) (w:world),
  infix_breq w (eq_variance p1 p2) <->
  infix_breq w (Know (infix_dleq (var p1) (var p2))).

Axiom Eq_variance_ref :
  forall (p:population) (w:world), infix_breq w (eq_variance p p).

Axiom Eq_variance_sym :
  forall (p1:population) (p2:population) (w:world),
  infix_breq w (eq_variance p1 p2) -> infix_breq w (eq_variance p2 p1).

Axiom Eq_variance_assoc :
  forall (p1:population) (p2:population) (p3:population) (w:world),
  infix_breq w (eq_variance p1 p2) /\ infix_breq w (eq_variance p2 p3) ->
  infix_breq w (eq_variance p1 p3).

Axiom Eq_variance_same_var :
  forall (m:real_number) (n:real_number) (v:real_number) (w:world),
  infix_breq w (eq_variance (NormalD m v) (NormalD n v)).

Parameter eq_mean: population -> population -> Prop.

Axiom Ppl_eq_mean :
  forall (p1:population) (p2:population),
  eq_mean p1 p2 <->
  (forall (w:world), infix_breq w (infix_dleq (mean p1) (mean p2))).

Axiom Refl8 : forall (x:population), eq_mean x x.

Axiom Trans8 :
  forall (x:population) (y:population) (z:population), eq_mean x y ->
  eq_mean y z -> eq_mean x z.

Axiom Symm8 :
  forall (x:population) (y:population), eq_mean x y -> eq_mean y x.

Axiom Eq_variance_same_mean :
  forall (m:real_number) (v:real_number) (v':real_number),
  eq_mean (NormalD m v) (NormalD m v').

(* Why3 assumption *)
Fixpoint aux (x:term) (ys:Init.Datatypes.list term)
  (pred:Strings.String.string) {struct ys}: Init.Datatypes.list formula :=
  match ys with
  | Init.Datatypes.nil => Init.Datatypes.nil
...
Axiom lem_aux_length :
  forall (x:term) (ys:Init.Datatypes.list term) (pred:Strings.String.string),
  ((list.Length.length (aux x ys pred)) = (list.Length.length ys)).

(* Why3 assumption *)
Fixpoint combinations (lst:Init.Datatypes.list term)
  (pred:Strings.String.string) {struct lst}: Init.Datatypes.list formula :=
  match lst with
  | Init.Datatypes.nil => Init.Datatypes.nil
...
Axiom lem_combinations_length :
  forall (lst:Init.Datatypes.list term) (pred:Strings.String.string),
  ((list.Length.length (combinations lst pred)) =
   (ZArith.BinInt.Z.quot
    ((list.Length.length lst) * ((list.Length.length lst) - 1%Z)%Z)%Z 2%Z)).

(* Why3 assumption *)
Fixpoint enumerate {a:Type} {a_WT:WhyType a} (l:Init.Datatypes.list a)
  (acc:Numbers.BinNums.Z) {struct l}: Init.Datatypes.list (Numbers.BinNums.Z*
                                      a)%type :=
  match l with
  | Init.Datatypes.nil => Init.Datatypes.nil
  | Init.Datatypes.cons x r =>
      Init.Datatypes.cons (acc, x) (enumerate r (acc + 1%Z)%Z)
  end.

(* Why3 assumption *)
Fixpoint aux_poly {a:Type} {a_WT:WhyType a} (x:a)
  (ys:Init.Datatypes.list a) {struct ys}: Init.Datatypes.list (a* a)%type :=
  match ys with
  | Init.Datatypes.nil => Init.Datatypes.nil
  | Init.Datatypes.cons y ys1 => Init.Datatypes.cons (x, y) (aux_poly x ys1)
  end.
...
(* Why3 assumption *)
Definition is_prob_dist (l:Init.Datatypes.list real_number) (w:world) : Prop :=
  match w with
  | World _ i =>
      let s :=
      fold
      (fun (acc:Reals.Rdefinitions.R) (r:real_number) =>
       (acc + (i (Real r)))%R)
      0%R l in ((99 / 100)%R <= s)%R /\ (s <= (101 / 100)%R)%R
  end.

(* Why3 assumption *)
Definition is_prob_dist_const (l:Init.Datatypes.list Reals.Rdefinitions.R) :
    Prop :=
  let s :=
  fold
  (fun (y0:Reals.Rdefinitions.R) (y1:Reals.Rdefinitions.R) => (y0 + y1)%R)
  0%R l in ((99 / 100)%R <= s)%R /\ (s <= (101 / 100)%R)%R.

Axiom is_prob_dist_const_implies_is_prob_dist_const :
  forall (s:Init.Datatypes.list Reals.Rdefinitions.R),
  is_prob_dist_const s <->
  (forall (w:world),
   is_prob_dist (map (fun (r:Reals.Rdefinitions.R) => Const r) s) w).

Axiom test_prob_dist :
  forall (w:world),
  is_prob_dist
  (Init.Datatypes.cons (Const (1 / 10)%R)
   (Init.Datatypes.cons (Const (9 / 10)%R) Init.Datatypes.nil))
  w.

Parameter rliteral10: Strings.String.string.

Axiom rliteral_axiom10 : True.

(* Why3 assumption *)
Definition infix_dleqcf (p1:population) (p2:population) : formula :=
  Atom
  (Pred rliteral10
   (Init.Datatypes.cons (PopulationT p1)
    (Init.Datatypes.cons (PopulationT p2) Init.Datatypes.nil))).
...
Parameter eq_real_number_list':
  world -> Init.Datatypes.list real_number ->
  Init.Datatypes.list real_number -> Prop.

Axiom eq_real_number_list''def :
  forall (w:world) (rs1:Init.Datatypes.list real_number)
    (rs2:Init.Datatypes.list real_number),
  match (rs1, rs2) with
  | (Init.Datatypes.cons h1 t1, Init.Datatypes.cons h2 t2) =>
      eq_real_number_list' w rs1 rs2 <->
      infix_breq w (infix_dleq (Real h1) (Real h2)) /\
      eq_real_number_list' w t1 t2
  | (Init.Datatypes.nil, Init.Datatypes.nil) =>
      eq_real_number_list' w rs1 rs2
  | _ => ~ eq_real_number_list' w rs1 rs2
  end.

Axiom eq_real_number_list'_refl :
  forall (p:Init.Datatypes.list real_number) (w:world),
  eq_real_number_list' w p p.

Axiom eq_real_number_list'_symm :
  forall (p1:Init.Datatypes.list real_number)
    (p2:Init.Datatypes.list real_number) (w:world),
  eq_real_number_list' w p1 p2 <-> eq_real_number_list' w p2 p1.

Axiom eq_real_number_list'_trans :
  forall (p1:Init.Datatypes.list real_number)
    (p2:Init.Datatypes.list real_number) (p3:Init.Datatypes.list real_number)
    (w:world),
...
Axiom eq_p_interp :
  forall (p1:population) (p2:population) (w:world),
  infix_breq w (infix_dleqcf p1 p2) <->
  match (p1, p2) with
  | (NormalD s1 s2, NormalD t1 t2) =>
      infix_breq w (infix_dleq (Real s1) (Real t1)) /\
      infix_breq w (infix_dleq (Real s2) (Real t2))
  | (BernoulliD s, BernoulliD t) =>
      infix_breq w (infix_dleq (Real s) (Real t))
  | (UnknownD i, UnknownD j) => (i = j)
...
Axiom eq_p_refl :
  forall (p:population) (w:world), infix_breq w (infix_dleqcf p p).

Axiom eq_p_symm :
  forall (p1:population) (p2:population) (w:world),
  infix_breq w (infix_dleqcf p1 p2) <-> infix_breq w (infix_dleqcf p2 p1).

Axiom eq_p_trans :
  forall (p1:population) (p2:population) (p3:population) (w:world),
  infix_breq w (infix_dleqcf p1 p2) /\ infix_breq w (infix_dleqcf p2 p3) ->
  infix_breq w (infix_dleqcf p1 p3).

Parameter rliteral11: Strings.String.string.

Axiom rliteral_axiom11 : True.

(* Why3 assumption *)
Definition infix_dlexeqcf (p1:population) (p2:population) : formula :=
  Atom
  (Pred rliteral11
   (Init.Datatypes.cons (PopulationT p1)
    (Init.Datatypes.cons (PopulationT p2) Init.Datatypes.nil))).
...
Axiom neq_p_def :
  forall (p1:population) (p2:population) (w:world),
  infix_breq w (infix_dlexeqcf p1 p2) <-> ~ infix_breq w (infix_dleqcf p1 p2).

Parameter x: Init.Datatypes.list real_number.

Parameter x1: Init.Datatypes.list real_number.

Parameter w: world.

Axiom H : eq_population (CategoricalD x) (CategoricalD x1).

(* Why3 goal *)
Theorem eq_population_soundness :
  infix_breq w (infix_dleqcf (CategoricalD x) (CategoricalD x1)).
Proof.
  rewrite eq_p_interp.
  assert (H : eq_real_number_list' w x x1).
  -- set H.
     unfold eq_population in e.
     exact e.
